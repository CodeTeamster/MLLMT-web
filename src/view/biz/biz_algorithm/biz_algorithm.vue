  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="算法名" prop="algorithmName" width="280" />

        <el-table-column align="left" label="创建人昵称" prop="creatorName" width="180" />
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateBizAlgorithmFunc(scope.row)">编辑</el-button>
...
const getTableData = async () => {
  const table = await getBizAlgorithmList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async () => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    const res = await deleteBizAlgorithmByIds({ IDs })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
...
// 更新行
const updateBizAlgorithmFunc = async (row) => {
  const res = await findBizAlgorithm({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteBizAlgorithmFunc = async (row) => {
  const res = await deleteBizAlgorithm({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
